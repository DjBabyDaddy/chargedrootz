...
- **Remove the noindex guards.** Three things keep the review build out of
  search results and must all go before launch: the
  `<meta name="robots" content="noindex, nofollow">` line in **both**
  `index.html` and `about.html` (each marked `REVIEW BUILD ONLY`),
  `robots.txt`, and `vercel.json`. Leaving any of them in place would keep
  the real site invisible to Google.
